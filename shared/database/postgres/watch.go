...
func monitorProgress(ctx context.Context, db *sql.DB, poll time.Duration, query string, args []any, label string) {
	go func() {
		start := time.Now()
...
func MonitorIndexCreationProgress(ctx context.Context, db *sql.DB, tableName, indexName string, poll time.Duration) {
	query := `SELECT
		p.phase,
		p.blocks_done,
		p.blocks_total,
	...
	monitorProgress(ctx, db, poll, query, []any{tableName}, fmt.Sprintf("CREATE INDEX '%s' ON '%s'", indexName, tableName))
}